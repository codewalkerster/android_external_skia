...
static void SCALE_NOFILTER_NAME(const SkBitmapProcState& s,
    ...
    if (count >= 8) {
    ...
    #else
        /*
         * tao.zeng@amlogic.com, unroll main loop to 16, adjust instruction 
         * order to prevent pipeline lock, increase efficient
         *
         * BUG tracker:
         * here we don't use vandq_s32, because if input are both positive
         * number, then (int32_t a + int32_t b) & 0xffff = (low 16 lsb of a + 
         * low 16 lsb of b), regardless of overflow, if one of input number are
         * negtive number, we don't validate this case, another case:we short maxX
         * to 16 bit number, because we havn't saw a picture have width more than
         * 65535 pixels
         *
         * NEON Register allocate:
         * d0 --> [fx + 3dx, fx + 2dx, fx + dx,  fx].16
         * d1 --> [fx + 7dx, fx + 6dx, fx + 5dx, fx + 4dx].16
         * d2 --> [8dx, 8dx, 8dx, 8dx].16
         * d3 --> [4dx, 4dx, 4dx, 4dx].16
         * d4 --> [maxX+1, maxX+1, maxX+1, maxX+1].16
         */
        asm volatile (
            "vdup.16    d2,    %[dx]                                \n" // d2 = [dx, dx, dx, dx].16
            "vdup.16    d4,    %[maxX]                              \n" // d4 = [maxX ...].16
            ...
            "sub        %[fx], %[fx], %[dx]                         \n"
            "sub        %[fx], %[fx], %[dx], lsl #1                 \n" // fx = fx_raw
            "blt        RepeatX_RepeatY_nofilter_scale_neon_less_than_16    \n"

        "RepeatX_RepeatY_nofilter_scale_neon_loop_16:               \n"
            "vmull.u16  q8, d0, d4                                  \n" // [fx + 3dx .. fx] * (maxX + 1)
            ...
            "add        %[fx], %[fx], %[dx], lsl #4                 \n" // fx += 16*dx
            "sub        %[count], %[count], #16                     \n" // count -= 16
            "vshrn.i32  d16, q8, #16                                \n" // result of [fx+ 3dx .. fx] * (maxX+1) 
            "vshrn.i32  d17, q9, #16                                \n" // result of [fx+ 7dx .. fx+ 4dx] * (maxX+1) 
            "vshrn.i32  d18, q10, #16                               \n" // result of [fx+11dx .. fx+ 8dx] * (maxX+1)
            "vshrn.i32  d19, q11, #16                               \n" // result of [fx+15dx .. fx+12dx] * (maxX+1)
            "cmp        %[count], #16                               \n"
            "vst1.16    {d16, d17, d18, d19}, [%[xy]]!              \n" // store data
            "bge        RepeatX_RepeatY_nofilter_scale_neon_loop_16 \n"

        "RepeatX_RepeatY_nofilter_scale_neon_less_than_16:          \n"
            "cmp        %[count], #8                                \n"
            "blt        RepeatX_RepeatY_nofilter_scale_neon_less_than_8 \n"
        ...
        "RepeatX_RepeatY_nofilter_scale_neon_less_than_8:           \n"
            "cmp        %[count], #4                                \n"
            "blt        RepeatX_RepeatY_nofilter_scale_neon_out     \n"
        ...
        "RepeatX_RepeatY_nofilter_scale_neon_out:                   \n"
            "nop                                                    \n"
            :[fx] "=r" (fx), [count] "=r" (count), [xy] "=r" (xy)
            :[maxX] "r" (maxX), [dx] "r" (dx)
            :"memory"
        );
    #endif
